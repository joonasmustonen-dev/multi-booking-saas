...
public class TenantJwtIntegrationTest {
    ...
    static class TestTenantController {
        ...
        TestTenantController(TenantRoutingDataSource routingDataSource) {
        ...
        String tenantDatabase() throws Exception {
            try (var connection = routingDataSource.getConnection()) {
    ...
    @Test
    void jwtTenantClaimCreatesTenantContextDuringRequest() throws Exception {
        mockMvc
            .perform(
                get("/api/test/tenant-context").with(
                    jwt().jwt(jwt -> jwt.claim("tenant_id", "tenant-a"))
                )
            )
    ...
    void jwtTenantClaimResolvesTenantB() throws Exception {
        mockMvc
            .perform(
                get("/api/test/tenant-context").with(
                    jwt().jwt(jwt -> jwt.claim("tenant_id", "tenant-b"))
                )
            )
            .andExpect(status().isOk())
            .andExpect(result -> {
        ...
        JwtDecoder jwtDecoder() {
            return token -> {
                throw new JwtException(
                    "Real JWT decoding is disabled in tests"
                );
            };
        }
    }
    ...
    @Test
    void authenticatedRequestWithoutTenantIsForbidden() throws Exception {
        mockMvc
            .perform(get("/api/test/tenant-database").with(jwt()))
    ...
    @Test
    void jwtTenantAReachesTenantADatabase() throws Exception {
        mockMvc
            .perform(
                get("/api/test/tenant-database").with(
                    jwt().jwt(jwt -> jwt.claim("tenant_id", "tenant-a"))
                )
            )
            .andExpect(status().isOk())
