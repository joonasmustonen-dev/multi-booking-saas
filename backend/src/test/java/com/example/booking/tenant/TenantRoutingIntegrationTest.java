...
public class TenantRoutingIntegrationTest {
    ...
    void tenantAAndTenantBUseDifferentDataSources() throws Exception {
        ...
        DataSource tenantA =
            dataSourceManager.getDataSource("tenant-a");

        DataSource tenantB =
            dataSourceManager.getDataSource("tenant-b");

        
        assertNotSame(tenantA, tenantB, "Tenant A and Tenant B must never share a DataSource");

        try (
                Connection connectionA =
                    tenantA.getConnection();
                Connection connectionB =
                    tenantB.getConnection()
        ) {
    ...
    void tenantAResolvesToTenantA() {
        ...
        assertEquals(
                "tenant-a",
                routingDataSource.currentLookupKeyForTest()
        );
    }
    ...
    void tenantAReadsOnlyTenantAData() throws Exception {
        ...
        try (
                ...
                var result =
                        statement.executeQuery("SELECT tenant_marker FROM routing_test_data")
        )  {
            ...
            assertEquals(
                    "THIS_IS_TENANT_A",
                    result.getString("tenant_marker")
            );
        }
    }
