...
class AvailabilityCombinationTest {
    ...
    private final TenantSettingsService settings = mock(TenantSettingsService.class);
    ...
    private final BookableResource resource = new BookableResource("Equipment", ResourceType.EQUIPMENT);
    private AvailabilityService service;

    @BeforeEach
    void setup() {
        service = new AvailabilityService(services, rules, exceptions, appointments, settings);
        when(services.findByIdWithResources(serviceId)).thenReturn(Optional.of(offering));
        when(settings.getZoneId()).thenReturn(ZoneOffset.UTC);
        when(settings.getSettings()).thenReturn(new com.example.booking.tenantdata.settings.TenantSettingsResponse("UTC"));
        ...
        when(rules.findAllByStaff_IdAndActiveTrue(staff.getId())).thenReturn(List.of(
                new AvailabilityRule(null, staff, null, date.getDayOfWeek(), LocalTime.of(9, 0), LocalTime.of(12, 0))));
        when(rules.findAllByLocation_IdAndActiveTrue(location.getId())).thenReturn(List.of(
                new AvailabilityRule(null, null, location, date.getDayOfWeek(), LocalTime.of(10, 0), LocalTime.of(13, 0))));
        when(rules.findByResourceIdAndActiveTrue(resource.getId())).thenReturn(List.of(
                new AvailabilityRule(resource, date.getDayOfWeek(), LocalTime.of(10, 30), LocalTime.of(11, 30))));
    }
    ...
    void intersectsHoursAndOmitsOptionalResource() {
        ...
        assertTrue(slots.stream().allMatch(s -> staff.getId().equals(s.staffId()) && location.getId().equals(s.locationId())));
        assertEquals(5, slots.stream().filter(s -> s.resourceId() == null).count());
    }
    ...
    void enumeratesDistinctStaffAndLocationCombinationsAndFiltersThem() {
        ...
        when(offering.getLocations()).thenReturn(Set.of(location, otherLocation));
        var staffRules = rules.findAllByStaff_IdAndActiveTrue(staff.getId());
        var locationRules = rules.findAllByLocation_IdAndActiveTrue(location.getId());
        when(rules.findAllByStaff_IdAndActiveTrue(otherStaff.getId())).thenReturn(staffRules);
        when(rules.findAllByLocation_IdAndActiveTrue(otherLocation.getId())).thenReturn(locationRules);
        var slots = find(null, null, null);
        assertEquals(24, slots.size());
        assertEquals(24, new HashSet<>(slots).size());
        assertEquals(6, find(otherStaff.getId(), otherLocation.getId(), null).size());
        verify(appointments, times(2)).findAssignmentBookingsInRange(any(), any(), any());
    }
    ...
    void bookingsOnAnyAssignmentBlockTheCombinationButTouchingBoundariesDoNot() {
        for (int dimension = 0; dimension < 3; dimension++) {
            ...
            when(booking.getLocation()).thenReturn(dimension == 1 ? location : null);
            when(booking.getResource()).thenReturn(dimension == 2 ? resource : null);
            when(booking.getStartAt()).thenReturn(at(10, 30));
            when(booking.getEndAt()).thenReturn(at(11, 30));
            when(appointments.findAssignmentBookingsInRange(any(), any(), any())).thenReturn(List.of(booking));
    ...
    void blockedExceptionWinsOverAvailableExceptionForEveryDimension() {
        for (int dimension = 0; dimension < 3; dimension++) {
            var blocked = new AvailabilityException(null, null, null, at(10, 30), at(11, 30), false);
            var open = new AvailabilityException(null, null, null, at(10, 0), at(12, 0), true);
            when(exceptions.findStaffExceptions(any(), any(), any())).thenReturn(dimension == 0 ? List.of(blocked, open) : List.of());
            when(exceptions.findLocationExceptions(any(), any(), any())).thenReturn(dimension == 1 ? List.of(blocked, open) : List.of());
            when(exceptions.findResourceExceptions(any(), any(), any())).thenReturn(dimension == 2 ? List.of(blocked, open) : List.of());
            assertTrue(find(null, null, resource.getId()).isEmpty());
        }
    }
    ...
    void openingExceptionCanSupplyHoursWhenRecurringRulesAreAbsent() {
        when(rules.findAllByStaff_IdAndActiveTrue(staff.getId())).thenReturn(List.of());
        when(exceptions.findStaffExceptions(any(), any(), any())).thenReturn(List.of(
                new AvailabilityException(null, staff, null, at(10, 0), at(11, 0), true)));
    ...
    void rejectsIneligibleFiltersAndDoesNotMakeInactiveStaffOptional() {
        for (int dimension = 0; dimension < 3; dimension++) {
            ...
            var error = assertThrows(ResponseStatusException.class,
                    () -> find(d == 0 ? unknown : null, d == 1 ? unknown : null, d == 2 ? unknown : null));
    ...
    void validatesDateRangeAndExcludesPastStarts() {
        assertThrows(ResponseStatusException.class,
                () -> service.findAvailability(serviceId, date, date.minusDays(1), null, null, null));
        assertThrows(ResponseStatusException.class,
                () -> service.findAvailability(serviceId, date, date.plusDays(32), null, null, null));
        LocalDate past = date.minusWeeks(2);
        assertTrue(service.findAvailability(serviceId, past, past, null, null, null).isEmpty());
    }

    @Test
    void endpointBindsAssignmentFiltersAndReturnsBookingIdentifiers() throws Exception {
        AvailabilityService apiService = mock(AvailabilityService.class);
        var response = new AvailabilitySlotResponse(staff.getId(), location.getId(), resource.getId(), at(10, 0), at(11, 0));
        when(apiService.findAvailability(serviceId, date, date, staff.getId(), location.getId(), resource.getId()))
                .thenReturn(List.of(response));
        MockMvc mvc = MockMvcBuilders.standaloneSetup(new AvailabilityController(apiService)).build();
        mvc.perform(get("/api/v1/availability").param("serviceId", serviceId.toString())
                        .param("from", date.toString()).param("to", date.toString())
                        .param("staffId", staff.getId().toString()).param("locationId", location.getId().toString())
                        .param("resourceId", resource.getId().toString()))
                .andExpect(status().isOk()).andExpect(jsonPath("$[0].staffId").value(staff.getId().toString()))
                .andExpect(jsonPath("$[0].locationId").value(location.getId().toString()))
                .andExpect(jsonPath("$[0].resourceId").value(resource.getId().toString()));
        mvc.perform(get("/api/v1/availability").param("serviceId", serviceId.toString())
                        .param("from", date.toString()).param("to", date.toString())).andExpect(status().isOk());
        verify(apiService).findAvailability(serviceId, date, date, null, null, null);
    }

    private List<AvailabilitySlotResponse> find(UUID staffId, UUID locationId, UUID resourceId) {
        return service.findAvailability(serviceId, date, date, staffId, locationId, resourceId);
    }
