...
class CustomerOperationsIntegrationTest {
    ...
    @Autowired
    ServiceOfferingService services;
    ...
    @Test
    void searchHandlesCaseFormattedPhonesLiteralWildcardsAndLimits() {
        inTenant(() -> {
            String token =
                "Lookup" + UUID.randomUUID().toString().replace("-", "");

            var first = customers.saveAndFlush(
                new Customer(
                    token,
                    "Alpha",
                    token + "@example.com",
            ...
            assertEquals(
                first.getId(),
                operations.search("040 123", 25).getFirst().id()
            );

            assertEquals(
                first.getId(),
                operations
                    .search(token + "@example.com", 25)
                    .getFirst()
                    .id()
            );
            ...
            assertFalse(
                operations
                    .search("%" + token, 25)
                    .stream()
                    .anyMatch(c -> c.id().equals(first.getId()))
            );
    ...
    @Test
    void preferredStaffPersistsClearsAndRejectsUnavailableAssignments() {
        inTenant(() -> {
            UUID staffId = staffService
                .create(new CreateAssignmentRequest("Preferred Sofia"))
                .id();
    ...
    @Test
    void historyReportsFactualStatusesAndOnlyCompletedPastVisitsAsUsualServices() {
        inTenant(() -> {
            UUID staffId = staffService
                .create(new CreateAssignmentRequest("History Sofia"))
                .id();

            var customer = customers.saveAndFlush(
                new Customer("History", "Customer", null, null)
            );

            UUID serviceId = services
                .create(
            ...
            for (int i = 0; i < statuses.length; i++) {
                ...
                var booking = new Appointment(
                    customer,
                    service,
                    staff.findById(staffId).orElseThrow(),
                    null,
                    null,
                    start,
                    start.plusHours(1),
                    "Do not expose appointment notes in customer history"
                );

                if (
                    statuses[i] != AppointmentStatus.CONFIRMED
                ) booking.changeStatus(statuses[i]);

                appointments.saveAndFlush(booking);

                if (statuses[i] == AppointmentStatus.COMPLETED) lastVisit =
                    booking.getEndAt();
            }
            ...
            assertEquals(1, result.mostBookedServices().size());

            assertEquals(2, result.mostBookedServices().getFirst().visits());
            ...
            assertEquals(
                AppointmentStatus.CONFIRMED,
                result.bookings().getFirst().status()
            );

            assertEquals(
                1,
                operations.activity(customer.getId(), 2, 2).bookings().size()
            );
            ...
            mvc.perform(
                get("/api/v1/customers/" + customer.getId() + "/activity")
            )
    ...
    @Test
    void customerOperationsStayInTenantAndRequireStaffRoles() {
        inTenant(() -> {
            String token = "Private" + UUID.randomUUID();

            var customer = customers.saveAndFlush(
                new Customer(token, "Customer", null, null)
            );
            ...
            try {
                ...
                isolated.executeWithoutResult(tx -> {
                    ...
                    assertEquals(
                        ...
                        assertThrows(ResponseStatusException.class, () ->
                            operations.activity(customer.getId(), 0, 20)
                        ).getStatusCode()
                    );
                });
            } finally {
            ...
            mvc.perform(
                get("/api/v1/customers/search").with(
                    jwt().authorities(
            ...
            mvc.perform(
                get("/api/v1/customers/" + customer.getId() + "/activity").with(
                    jwt().authorities(
                        new SimpleGrantedAuthority("ROLE_CUSTOMER")
                    )
                )
    ...
    @Test
    void emptyCustomerAndInvalidHistoryParametersAreHandled() {
        inTenant(() -> {
            var customer = customers.saveAndFlush(
                new Customer("New", "Customer", null, null)
            );
            ...
            assertNull(result.lastVisit());

            assertTrue(result.mostBookedServices().isEmpty());
