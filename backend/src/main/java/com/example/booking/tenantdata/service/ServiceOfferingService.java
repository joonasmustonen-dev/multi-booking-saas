...
public class ServiceOfferingService {
    ...
    public ServiceResponse create(CreateServiceRequest request) {
        ...
        service.replaceStaff(resolveStaff(request.staffIds()));

        service.replaceLocations(resolveLocations(request.locationIds()));

        service.replaceResources(resolveResources(request.resourceIds()));

        service.configureRequirements(
            request.staffRequirement(),
            request.locationRequirement(),
    ...
    private Set<BookableResource> resolveResources(Set<UUID> ids) {
        if (ids == null || ids.isEmpty()) {
            return Set.of();
        }

        Set<BookableResource> result = new HashSet<>(
            resourceRepository.findAllById(ids)
        );
        ...
        for (BookableResource resource : result) {
            if (
    ...
    private Set<StaffMember> resolveStaff(Set<UUID> ids) {
        if (ids == null || ids.isEmpty()) {
            return Set.of();
        }

        Set<StaffMember> result = new HashSet<>(
            staffRepository.findAllById(ids)
        );
    ...
    private Set<Location> resolveLocations(Set<UUID> ids) {
        if (ids == null || ids.isEmpty()) {
            return Set.of();
        }

        Set<Location> result = new HashSet<>(
            locationRepository.findAllById(ids)
        );
