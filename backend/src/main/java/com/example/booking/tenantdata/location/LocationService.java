...
public class LocationService {
    ...
    public LocationResponse create(CreateAssignmentRequest request) {
        return LocationResponse.from(
            repository.save(new Location(request.name()))
        );
    }
    ...
    public List<LocationResponse> findAll() {
        return repository
            ...
            .stream()
            .sorted(
                Comparator.comparing(Location::getName).thenComparing(
                    Location::getId
                )
            )
    ...
    public LocationResponse findById(UUID id) {
        return LocationResponse.from(entity(id));
    }
    ...
    public LocationResponse create(LocationRequest request) {
        var entity = new Location(request.name().trim());

        applyDetails(entity, request);
    ...
    public LocationResponse update(UUID id, LocationRequest request) {
        ...
        entity.update(
            request.name().trim(),
            request.active() == null ? entity.isActive() : request.active()
        );
    ...
    private void applyDetails(Location entity, LocationRequest request) {
        if (request.active() != null) entity.update(
            entity.getName(),
            request.active()
        );
        ...
        if (request.addressLine() != null) entity.setAddressLine(
            request.addressLine().trim()
        );

        if (request.city() != null) entity.setCity(request.city().trim());

        if (request.postalCode() != null) entity.setPostalCode(
            request.postalCode().trim()
        );

        if (request.countryCode() != null) entity.setCountryCode(
