...
public class AvailabilityScheduleService {
    ...
    public List<AvailabilityRuleResponse> findRules(
        ...
        List<AvailabilityRule> result = switch (type) {
            case STAFF -> rules.findByStaff_IdOrderByDayOfWeekAscStartTimeAsc(
                id
            );
            case LOCATION -> rules.findByLocation_IdOrderByDayOfWeekAscStartTimeAsc(
                id
            );
            case RESOURCE -> rules.findByResource_IdOrderByDayOfWeekAscStartTimeAsc(
                id
            );
        };
    ...
    public void deleteRule(AvailabilityOwnerType type, UUID id, UUID ruleId) {
        ...
        rules.delete(rule(type, id, ruleId));
    }
    ...
    public List<AvailabilityRuleResponse> replaceRules(
        ...
        for (var draft : request.rules())
            validateTimes(draft.startTime(), draft.endTime());

        for (DayOfWeek day : DayOfWeek.values()) {
            var active = request
                .rules()
                .stream()
                .filter(r -> r.dayOfWeek() == day && r.active())
                .sorted(
                    java.util.Comparator.comparing(
                        ReplaceAvailabilityRulesRequest.Rule::startTime
                    )
                )
                .toList();
            ...
            for (var draft : active) {
                if (
                    previousEnd != null &&
                    draft.startTime().isBefore(previousEnd)
                ) throw new ResponseStatusException(
                    HttpStatus.BAD_REQUEST,
                    "Recurring hours overlap on " + day
                );
    ...
    public AvailabilityExceptionResponse createException(
        ...
        return AvailabilityExceptionResponse.from(
            exceptions.save(
    ...
    public List<AvailabilityExceptionResponse> findExceptions(
        ...
        return result
            .stream()
            .filter(exception -> exception.getScheduleWeek() == null)
    ...
    public void deleteException(
        ...
        exceptions.delete(exception(type, id, exceptionId));
    }
    ...
    private AvailabilityRule rule(
        ...
        var rule = rules
            .findById(ruleId)
            .orElseThrow(() -> missing("Availability rule"));
        ...
        ) throw missing("Availability rule");
        return rule;
    }
    ...
    private AvailabilityException exception(
        ...
        var exception = exceptions
            ...
            .orElseThrow(() -> missing("Availability exception"));
        ...
        ) throw missing("Availability exception");
        if (
            exception.getScheduleWeek() != null
        ) throw new ResponseStatusException(
            HttpStatus.BAD_REQUEST,
            "Edit this exception using the weekly staff rota"
        );
        return exception;
    }
    ...
    private Owner owner(AvailabilityOwnerType type, UUID id) {
        return switch (type) {
            case STAFF -> new Owner(
                null,
                staff
                    .findById(id)
                    .filter(member -> !member.isRemoved())
                    .orElseThrow(() -> missing("Staff member")),
                null
            );
            case LOCATION -> new Owner(
                null,
                null,
                locations.findById(id).orElseThrow(() -> missing("Location"))
            );
            case RESOURCE -> {
                var resource = resources
                    .findById(id)
                    .orElseThrow(() -> missing("Resource"));
                ...
                if (
                    ...
                    HttpStatus.BAD_REQUEST,
                    "Use native staff or location availability"
                );
    ...
    private void validateTimes(LocalTime start, LocalTime end) {
        if (!end.isAfter(start)) throw new ResponseStatusException(
            HttpStatus.BAD_REQUEST,
            "endTime must be after startTime"
        );
    }

    private ExceptionTimes exceptionTimes(
        AvailabilityExceptionRequest request
    ) {
        ZoneId zone = settings.getZoneId();

        if (
            zone.getRules().getValidOffsets(request.startAt()).isEmpty() ||
            zone.getRules().getValidOffsets(request.endAt()).isEmpty()
        ) throw new ResponseStatusException(
            HttpStatus.BAD_REQUEST,
            "Exception time does not exist in the tenant timezone"
        );
