...
export default function ResourcesPage() {
    ...
    const query = useQuery({ queryKey: ["resources"], queryFn: getResources });
    const [editing, setEditing] = useState<BookableResource | "new" | null>(
        null
    );
    const [pending, setPending] = useState(false);
    const [error, setError] = useState("");
    async function save(values: ResourceRequest) {
    ...
    async function mutate(action: () => Promise<unknown>) {
        ...
        try {
            ...
            setError(
                e instanceof Error ? e.message : "Could not update resource."
            );
        } finally {
            setPending(false);
        }
    }
    if (query.isPending || settings.isPending) return <p>Loading resources…</p>;
    ...
    return (
        <div className="management-page">
            <div className="page-header">
                <div>
                    <p className="page-eyebrow">Tools for every booking</p>
                    <h1 className="page-title">Resources</h1>
                    <p className="page-description">
                        Manage your equipment, vehicles, and other bookable
                        things.
                    </p>
                </div>
                <button
                    ...
                    {visible => (
                        <div className="service-grid">
                            {visible.map(resource => (
                                <article
                                    className="card service-card resource-card"
                                    key={resource.id}
                                >
                                    <div className="section-heading">
                                        ...
                                        <span
                                            className={`requirement-pill ${resource.active ? "sage" : "muted"}`}
                                        >
                                            {resource.active
                                                ? "Active"
                                                : "Inactive"}
                                    ...
                                    {resource.description && (
                                        <p className="field-note resource-description">
                                            {resource.description}
                                        </p>
                                    )}
