...
export function toggleRecurringDay(
    ...
    if (!rules.some(rule => rule.dayOfWeek === day))
        return [
            ...rules,
            { dayOfWeek: day, startTime: "09:00", endTime: "17:00", active }
        ];
    return rules.map(rule =>
        rule.dayOfWeek === day ? { ...rule, active } : rule
    );
}
...
export function validateRecurringDraft(rules: RecurringDraft[]) {
    ...
    if (
        ...
        return "Every time period must end after it starts.";
    for (const day of weekDays) {
        const active = rules
            .filter(rule => rule.dayOfWeek === day && rule.active)
            .sort((a, b) => a.startTime.localeCompare(b.startTime));
        if (
            active.some(
                (rule, index) =>
                    index > 0 && rule.startTime < active[index - 1].endTime
            )
        )
            return `Time periods overlap on ${day.toLowerCase()}.`;
    }
    return "";
}
