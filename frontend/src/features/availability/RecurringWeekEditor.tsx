    ...
    return (
        <form className="recurring-week" onSubmit={save}>
            <p className="field-note">
                Click a day to open or close it, then set its times. Changes
                apply when you save.
            </p>
            <fieldset className="editor-fields" disabled={pending}>
                <div className="recurring-day-strip">
                    {weekDays.map(day => {
                        ...
                        return (
                            <button
                                ...
                                onClick={() =>
                                    change(
                                        toggleRecurringDay(rules, day, !active)
                                    )
                                }
                            >
                                <span>
                ...
                <div className="recurring-presets">
                    ...
                    <button
                        ...
                        onClick={() =>
                            change(
                                rules.map(rule => ({ ...rule, active: false }))
                            )
                        }
                    >
                        Close all days
                ...
                <div className="recurring-day-rows">
                    {weekDays
                        .filter(day =>
                            rules.some(
                                rule => rule.dayOfWeek === day && rule.active
                        ...
                        .map(day => (
                            <div className="recurring-day-row" key={day}>
                                <strong>
                                    {day.charAt(0) + day.slice(1).toLowerCase()}
                                </strong>
                                <div className="recurring-shifts">
                                    {rules
                                        .map((rule, index) => ({ rule, index }))
                                        .filter(
                                            entry =>
                                                entry.rule.dayOfWeek === day
                                        )
                                        .map(({ rule, index }) => (
                                            <div
                                                className="recurring-time-row"
                                                key={index}
                                            >
