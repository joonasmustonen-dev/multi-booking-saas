...
export default function ServicesPage() {
    ...
    const [editing, setEditing] = useState<ServiceOffering | "new" | null>(
        null
    );
    const save = useMutation({
        mutationFn: ({
            ...
            id?: string;
            values: UpdateServiceRequest;
        }) => (id ? updateService(id, values) : createService(values)),
        onSuccess: async () => {
    ...
    const remove = useMutation({
        ...
        onSuccess: () => invalidateBookingData(client)
    });
    if (services.isPending || catalogs.isPending || settings.isPending)
        return <p>Loading services…</p>;
    if (services.error || catalogs.error || settings.error)
        return (
            <p className="form-error" role="alert">
                {(services.error ?? catalogs.error ?? settings.error)?.message}
            </p>
        );
    ...
    return (
        <div className="management-page">
            <div className="page-header">
                <div>
                    <p className="page-eyebrow">Your offering</p>
                                ...
                                >
                                    ...
                                    {(
                                        [
                                            [
                                                "Staff",
                                                service.staffRequirement,
                                                service.staffIds,
                                                catalogs.data.staff
                                            ],
                                            [
                                                "Locations",
                                                service.locationRequirement,
                                                service.locationIds,
                                            ...
                                            [
                                                ...
                                                service.resourceIds,
                                                catalogs.data.resources
                                            ]
                                        ] as const
                                    ).map(
                                        ([label, requirement, ids, items]) => (
                                            <div
                                                className="service-assignment"
                                                key={label}
                                            >
                                                <div>
                                                    <strong>{label}</strong>
