...
export default function CustomerForm({
    ...
    const options = [
        { value: "", label: "No staff preference" },
        ...(staff.data ?? [])
            .filter(s => s.active || s.id === preferredStaffId)
            .map(s => ({
                value: s.id,
                label: s.name,
                description: s.active
                    ? s.email
                    : "Inactive · existing preference"
            }))
    ];
    if (preferredStaffId && !options.some(s => s.value === preferredStaffId))
        options.push({
            value: preferredStaffId,
            label: "Previous staff preference",
            description: "Archived or unavailable"
        });
    return (
        <form className="customer-form management-form" onSubmit={submit}>
            ...
            <label className="form-field">
                ...
                <SearchSelect
                    ...
                    options={options}
                    ariaLabel="Customer preferred staff"
                    disabled={staff.isPending || !!staff.error}
                />
            </label>
            <p className="field-note">
                An informational preference; service eligibility and
                availability still determine booking options.
            </p>
            {staff.error && (
                <p className="form-error">
                    Staff preferences could not be loaded. Existing preferences
                    will be preserved.
                </p>
            )}
            {error && (
