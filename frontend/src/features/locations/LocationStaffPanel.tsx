...
export default function LocationStaffPanel({
    locationId,
    locationName
}: {
    locationId: string;
    ...
    async function update(
        ...
        try {
            await saveStaff(
                { ...staffValues(member), freeAgent, locationIds },
                member.id
            );
            ...
            setError(
                e instanceof Error
                    ? e.message
                    : "Could not update assigned staff."
            );
        } finally {
            setPending(false);
        }
    }
    if (query.isPending) return <p>Loading assigned staff…</p>;
    if (query.error)
        return (
    ...
    const assigned = query.data.filter(
        member => !member.freeAgent && member.locationIds.includes(locationId)
    );
    const freeAgents = query.data.filter(
        member => member.freeAgent && member.active
    );
    const candidates = query.data.filter(
        member =>
            member.active &&
            !assigned.some(assigned => assigned.id === member.id)
    );
    ...
    return (
        <section className="card management-panel tinted-panel sage-panel">
            ...
            <div className="assigned-team">
                {assigned.length === 0 ? (
                    <p className="field-note">
                        No staff are limited to this location.
                    </p>
                ) : (
                    assigned.map(member => (
                        <div className="assigned-person" key={member.id}>
                            <span className="staff-avatar">
                                {member.name.slice(0, 1)}
                            </span>
                            <div>
                                <strong>
                                    {entityLabel(member, query.data)}
                                </strong>
                                <small>
                                    {member.email ||
            ...
            <div className="free-agent-group">
                <p className="detail-line">
                    <MapPin size={16} />
                    Free agents can also work here
                </p>
                <div className="selected-tags">
                    {freeAgents.length === 0 ? (
                        <span className="field-note">
                            No active free agents.
                        </span>
                    ) : (
...
Available at any eligible service location, subject to their schedule.`}
                            >
                                {member.name}
                            </span>
                        ))
