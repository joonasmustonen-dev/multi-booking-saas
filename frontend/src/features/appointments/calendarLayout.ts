import type { AppointmentCalendarItem } from "./appointmentTypes";
...
export interface DayCalendarLayout {
    ...
    clusters: AppointmentOverlapCluster[];
}
...
export function minutesInTimeZone(iso: string, timeZone: string) {
    const parts = new Intl.DateTimeFormat("en-US", {
        timeZone,
...
export function layoutDayAppointments(
    ...
    const ordered = appointments
        .map(appointment => ({
            ...
            startMinute: minutesInTimeZone(appointment.startAt, timeZone),

            endMinute: minutesInTimeZone(appointment.endAt, timeZone)
        }))
        .sort((first, second) => {
     ...
     */
    const rawClusters: (typeof ordered)[] = [];

    let currentCluster: typeof ordered = [];
    ...
    for (const appointment of ordered) {
        if (
            ...
            appointment.startMinute < currentClusterEnd
        ) {
            currentCluster.push(appointment);

            currentClusterEnd = Math.max(
                currentClusterEnd,
        ...
        currentClusterEnd = appointment.endMinute;
    }
    ...
    const clusters: AppointmentOverlapCluster[] = [];

    rawClusters.forEach((cluster, clusterId) => {
        /*
        ...
        const laneAssignments = cluster.map(item => {
        ...
        for (const item of laneAssignments) {
            positioned.push({
        ...
        clusters.push({
            ...
            startMinute: Math.min(...cluster.map(item => item.startMinute)),

            endMinute: Math.max(...cluster.map(item => item.endMinute)),
