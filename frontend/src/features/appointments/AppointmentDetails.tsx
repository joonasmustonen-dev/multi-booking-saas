...
interface Props {
    ...
    onStatus: (status: AppointmentStatus) => Promise<unknown>;
}
...
export default function AppointmentDetails({
    ...
    const customer = useQuery({
        queryKey: ["customer", appointment.customerId],
        queryFn: () => getCustomer(appointment.customerId)
    });
    ...
    const panel = (
        <div className="appointment-drawer-backdrop" onMouseDown={onClose}>
            <aside
                className="appointment-drawer appointment-detail-panel"
                role="dialog"
                aria-modal="true"
            ...
            >
                <header className="appointment-detail-hero">
                    <div className="section-heading">
                        <span className="page-eyebrow">Booking details</span>
                        <button
                            ref={close}
                    ...
                    <div className="appointment-customer-heading">
                ...
                <div className="appointment-detail-body">
                    ...
                    <div className="appointment-assignment-tiles">
                        <div>
                            <UserRound size={19} />
                            <small>Staff member</small>
                            <strong>
                                {appointment.staffName || "No staff assigned"}
                            </strong>
                        </div>
                        <div>
                            <MapPin size={19} />
                        ...
                        <div>
                            ...
                            <strong>
                                {appointment.resourceName || "No resource"}
                            </strong>
                        </div>
                        {service?.price != null && (
                            <div>
                        ...
                        {customer.isPending ? (
                            ...
                            <div className="appointment-contact-list">
                                ...
                                <div>
                                    <Phone size={16} />
                                    {customer.data.phone ? (
                                        <a href={`tel:${customer.data.phone}`}>
                                            {customer.data.phone}
                                        </a>
                                    ) : (
                    ...
                    <section className="appointment-info-section">
                        ...
                        <p
                            ...
                            tabIndex={0}
                            role="region"
                            aria-label="Appointment notes: hover or focus to scroll"
                        >
                            {detail.isPending
                                ? "Loading notes…"
                                : detail.error
                                  ? "Notes could not be loaded."
                                  : detail.data.notes || "No notes added."}
                        </p>
                        {((detail.data?.notes?.length ?? 0) > 240 ||
                            (detail.data?.notes?.split("\n").length ?? 0) >
                                4) && (
                            <small className="field-note">
                                Hover or focus the notes to scroll.
                            </small>
                        )}
                    </section>
                    {detail.data?.createdAt && (
                        <p className="field-note">
                            Booked {formatDate(detail.data.createdAt, timeZone)}
                        </p>
                    )}
                    {error && (
                ...
                <footer className="appointment-detail-actions">
                    {pending && (
                    ...
                    {appointment.status === "PENDING" && (
                        <button
                            ...
                            disabled={pending}
                            onClick={() => act(() => onStatus("CONFIRMED"))}
                        >
                            <Check size={16} />
                            Confirm booking
                        </button>
                    ...
                    {appointment.status === "CONFIRMED" && (
                        <>
                            <button
                                className="button button-primary"
                                disabled={pending}
                                onClick={() => act(() => onStatus("COMPLETED"))}
                            >
                                <CheckCheck size={16} />
                                Mark completed
                            </button>
                    ...
                    {!active && (
                        <p className="field-note">
                            This appointment is{" "}
                            {statusNames[appointment.status].toLowerCase()}.
                        </p>
                    )}
                </footer>
            </aside>
