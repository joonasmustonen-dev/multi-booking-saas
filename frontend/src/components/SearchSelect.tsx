...
export interface SelectOption { value: string; label: string; description?: string; disabled?: boolean; tone?: string }
...
export default function SearchSelect({ value, options, onChange, placeholder = "Select…", ariaLabel, searchable = true, disabled = false, required = false, className = "", onSearchChange, filterLocally = true, loading = false, selectedLabel }: Props) {
    ...
    const place = useCallback(() => {
        ...
        const below = window.innerHeight - rect.bottom - 16; const above = rect.top - 16;
        const maxHeight = Math.min(280, Math.max(80, below < 160 && above > below ? above : below));
        const visibleHeight = Math.min(maxHeight, menu.current?.scrollHeight ?? (filtered.length * 48 + (searchable ? 35 : 12)));
    ...
    useEffect(() => {
        if (!open) return;
        place();
        const closeOutside = (event: PointerEvent) => { const target = event.target as Node; if (!anchor.current?.contains(target) && !menu.current?.contains(target)) setOpen(false); };
        document.addEventListener("pointerdown", closeOutside); window.addEventListener("resize", place); window.addEventListener("scroll", place, true);
        return () => { document.removeEventListener("pointerdown", closeOutside); window.removeEventListener("resize", place); window.removeEventListener("scroll", place, true); };
    }, [open, query, options.length, place]);
    function keyboard(event: KeyboardEvent) {
        if (event.key === "Escape") { setOpen(false); setQuery(""); event.preventDefault(); return; }
        if (event.key === "Tab") { setOpen(false); setQuery(""); return; }
    ...
    return <div ref={anchor} className={`search-select ${open ? "is-open" : ""} ${className}`}>
        {searchable ? <><Search size={16} className="search-select-icon" /><input {...common} aria-autocomplete="list" required={required} value={open ? query : selected?.label ?? selectedLabel ?? ""} placeholder={selected?.label ?? selectedLabel ?? placeholder} onFocus={show} onClick={() => { if (!open) show(); }} onBlur={() => { setOpen(false); setQuery(""); }} onChange={e => { setQuery(e.target.value); onSearchChange?.(e.target.value); setHighlight(0); }} /></> : <button {...common} type="button" onClick={() => open ? setOpen(false) : show()}><span>{selected?.tone && <i className={`status-dot status-${selected.tone}`} />}{selected?.label ?? selectedLabel ?? placeholder}</span></button>}
        ...
        {open && typeof document !== "undefined" && createPortal(<div ref={menu} id={`${id}-menu`} role="listbox" aria-label={ariaLabel} className="select-popover" style={position}>
            ...
            {filtered.length === 0 ? <p className="select-empty">No matching options</p> : filtered.map((option, index) => <button type="button" tabIndex={-1} id={`${id}-${index}`} role="option" aria-selected={value === option.value} disabled={disabled || loading || option.disabled} key={option.value} className={`select-option ${option.tone ? "status-" + option.tone : ""} ${highlight === index ? "highlighted" : ""}`} onMouseDown={e => e.preventDefault()} onMouseEnter={() => setHighlight(index)} onClick={() => choose(option)}><span><strong>{option.tone && <i className={`status-dot status-${option.tone}`} />}{option.label}</strong>{option.description && <small>{option.description}</small>}</span>{value === option.value && <Check size={16} />}</button>)}
        </div>, document.body)}
    </div>;
}
export function SearchMultiSelect({ values, options, onChange, ariaLabel, disabled = false }: { values: string[]; options: SelectOption[]; onChange: (values: string[]) => void; ariaLabel: string; disabled?: boolean }) {
    return <div className="multi-select"><div className="selected-tags">{values.map(value => <span className="selected-tag" key={value}>{options.find(option => option.value === value)?.label ?? "Unavailable location"}<button type="button" disabled={disabled} aria-label={`Remove ${options.find(option => option.value === value)?.label ?? "location"}`} onClick={() => onChange(values.filter(item => item !== value))}><X size={12} /></button></span>)}</div><SearchSelect value="" options={options.filter(option => !values.includes(option.value))} onChange={value => onChange([...values, value])} ariaLabel={ariaLabel} disabled={disabled} placeholder="Type to add a location…" /></div>;
}
